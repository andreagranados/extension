                ...
                foreach ($integrantes as $integrante) {
                    ...
                    if ($integrante[tipo] == 'Docente') {
                        if ($i == 0) {
                            ...
                            $pdf->ezTable($datos_pext, array('col1' => ''), ' ', array('showHeadings' => 0, 'shaded' => 2, 'width' => 550, 'cols' => array('col1' => array('justification' => 'center', 'width' => 580))));
                            $cols_dp = array('col1' => "<b> Nro </b>", 'col2' => '<b>' . utf8_d_seguro('Función') . '</b>', 'col3' => 'Nombre y Apellido', 'col4' => utf8_d_seguro('Documento'), 'col5' => utf8_d_seguro('Unidad Academica'), 'col6' => utf8_d_seguro('e-mail'), 'col7' => 'Carga Horaria');
                            $tabla_dp = array();
                        }

                        $tabla_dp[$i] = array('col1' => $i, 'col2' => $integrante[funcion_p], 'col3' => $integrante[nombre], 'col4' => $integrante[tipo_docum] . '' . $integrante[nro_docum], 'col5' => $integrante[ua], 'col6' => $integrante[mail], 'col7' => $integrante[carga_horaria]);

                        $i = $i + 1;
                    }
                }
                if (count($tabla_dp) >= 1) {
                    $pdf->ezTable($tabla_dp, $cols_dp, '', array('shaded' => 0, 'showLines' => 2, 'width' => 550, 'cols' => array('col1' => array('justification' => 'center', 'width' => 30), 'col2' => array('width' => 50), 'col3' => array('width' => 115), 'col4' => array('width' => 80), 'col5' => array('width' => 60), 'col6' => array('width' => 200),'col7' => array('justification' => 'center', 'width' => 45))));
                }
            ...
            if($band){
                if($datos['tipo_cambio']=='P'){
                    if($datos['fecha_fin_prorroga']>$hasta){
                        ...
                        toba::notificacion()->agregar(utf8_decode('También se ha modificado la fecha de hasta de los integrantes '),'info');   
                    }else{
                        $guardar=false;
...
//        }
    ...
    function conf__form_integrantes(toba_ei_formulario $form) {
        if ($this->s__mostrar == 1) {
            $perfil = toba::manejador_sesiones()->get_perfiles_funcionales()[0];
            $estado = $this->dep('datos')->tabla('pextension')->get()[id_estado];
            // solo en FORM o MODF el formulador puede modif o baja, o la sec ua en estado APRB o PRG
            if (!(($perfil == 'formulador' and ($estado == 'FORM' or $estado == 'MODF')) or ($perfil=='sec_ext_ua' and ($estado=='APRB' or $estado == 'PRG'))) ) {
...
//        }
    ...
    function conf__form_integrante_e(toba_ei_formulario $form) {
        if ($this->s__mostrar_e == 1) {
            ...
            //solo en estado FORM o MODF el formulador puede modificar/bajar o si es la sec ua en APRB o PRG
            if (!(($perfil == 'formulador' and ($estado == 'FORM' or $estado == 'MODF')) or ($perfil=='sec_ext_ua' and ($estado=='APRB' or $estado == 'PRG'))) ) {
...
//        }
    ...
    function conf__form_objetivos_esp(toba_ei_formulario $form) {
        ...
        if ($this->s__mostrar_obj == 1) {
            $estado = $this->dep('datos')->tabla('pextension')->get()[id_estado];
            // solo en estado FORM o MODF el formulador puede modif 
            if (!(($perfil == 'formulador' and ($estado == 'FORM' or $estado == 'MODF')) or ($perfil=='sec_ext_ua' and ($estado=='APRB' or $estado == 'PRG'))) ) {
