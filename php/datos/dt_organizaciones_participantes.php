...
class dt_organizaciones_participantes extends extension_datos_tabla {
    ...
    function cargarTablasTemporales(){
        ...
        for ($i=0; $i < 3 ; $i++) { 
            foreach ($res[$i] as $datos) {
                ...
                if ($i == 0){
                    $query = "INSERT INTO pg_temp.tabla_temporal_pais (pais) VALUES ('$datos_json')"; 
                }
                if ($i == 1){
                    $query = "INSERT INTO pg_temp.tabla_temporal_provincia (provincia) VALUES ('$datos_json')"; 
                }
                if ($i == 2){
                    $query = "INSERT INTO pg_temp.tabla_temporal_localidad (localidad) VALUES ('$datos_json')"; 
                }
                toba::db('extension')->consultar($query);
            }
        }
    ...
    function get_listado_filtro($id, $filtro = array()) {
        ...
        $sql = "SELECT
                ...
                INNER JOIN 
                    pextension AS p_e ON (o_p.id_pext = p_e.id_pext)
                LEFT OUTER JOIN 
                (SELECT 
                ...
                LEFT OUTER JOIN 
                    tipo_organizacion AS t_o ON (o_p.id_tipo_organizacion = t_o.id_tipo_organizacion)
                WHERE o_p.id_localidad = temp_local.id";
