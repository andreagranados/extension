 ...
 */
class recurso_aprbadosord implements SIUToba\rest\lib\modelable # Esta interface es documentativa, puede no estar
{
    ...
    protected function get_spec_ordenado($m = null, $tipo = 'Proyecto')
    {
        $m = $this->_get_modelos();
        return $m[$tipo];
    }
    ...
    function get_list()
    {
        ...
        $proyectos = modelo_pextension::get_ordenados($limit);
        ...
        $proyectos = rest_hidratador::hidratar($this->get_spec_ordenado(false), $proyectos);
        $cantidad = modelo_pextension::get_cant_odenados($where);
